...
# ==========================================
# 3. STAFF CATALOG (NATIVE STREAMLIT PURE)
# ==========================================
else:
    periode_sekarang = datetime.now().strftime("%B%Y")
    ...
    if url_sheet and "http" in url_sheet:
        try:
            ...
            # PENCARIAN NATIVE STREAMLIT
            search_query = st.text_input("🔍 Cari Barang:", placeholder="Ketik nama barang di sini...").strip().lower()

            filtered_df = df_barang.copy()
            if search_query:
                filtered_df = filtered_df[filtered_df['Nama Barang'].astype(str).str.lower().str.contains(search_query)]

            st.caption(f"Menampilkan {len(filtered_df)} barang")

            # CONTAINER DAFTAR BARANG (MAX 15 BARANG PER TAMPILAN)
            with st.container():
                for idx, row in filtered_df.head(15).iterrows():
                    nama = str(row['Nama Barang'])
                    satuan = str(row['Satuan'])
                    harga_raw = row['Harga']
                    harga_clean = float(re.sub(r'[^0-9]', '', str(harga_raw))) if pd.notnull(harga_raw) else 0

                    with st.container():
                        st.markdown(f"""
                        <div class="item-card-native">
                        ...
                        with c_btn:
                            if st.button("➕ Tambah", key=f"btn_add_{idx}"):
                                ...
                                add_subtotal = harga_clean * qty_val

                                # Cek item jika sudah ada di keranjang
                                found = False
                                ...
                                st.success(f"✅ {nama} (x{qty_val}) ditambahkan!")
                                st.rerun()
